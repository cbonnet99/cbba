class Admin::UsersController < AdminApplicationController
  ...
  def renewals
    ...
    @count_renewed_payments_in_last_year = Payment.count(:all, :conditions => ["status = 'completed' and first_time IS TRUE and created_at between ? and ? ", 365.days.ago, Time.now])
    ...
    @renewed_payments_in_past_month = Payment.find(:all, :include => "user", :conditions => ["status = 'completed' and first_time IS TRUE and created_at between ? and ? ", 30.days.ago, Time.now])
    ...
    @expiring_users_in_coming_month = User.find(:all, :conditions => ["paid_photo_until between ? and ?", Time.now, 30.days.from_now], :order => "paid_photo_until")
  end
