  ...
  xml.channel do
    ...
    for article in @articles
      xml.item do
        ...
        xml.description "by #{author_link(article.author)}<br/>#{white_list(article.lead)}<br/>#{white_list(article.body)}<br/><h4>About the author</h4>
        <%= white_list article.about %><br/>#{author_link(article.author, 'Meet '+article.author.try(:name))}"
