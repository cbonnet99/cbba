...
class TabTest < ActiveSupport::TestCase
  ...
  def test_set_contents_missing_title
    user = Factory(:user)
    tab = Factory(:tab, :user => user, :title => "Yoga", :content => "<h3>Yoga with #{user.name}</h3>CONTENT1<h3>Some other random title</h3>RANDOM CONTENT<h3>#{user.first_name}'s Training</h3>CONTENT3<h3>About #{user.name}</h3>CONTENT4" )
    tab.reload
  ...
  def test_set_contents_mix
    user = Factory(:user)
    tab = Factory(:tab, :user => user, :title => "Yoga", :content => "<h3>About #{user.name}</h3>CONTENT4<h3>Benefits of Yoga</h3>CONTENT2<h3>#{user.first_name}'s Training</h3>CONTENT3<h3>Yoga with #{user.name}</h3>CONTENT1" )
    tab.reload
    assert_equal "CONTENT1", tab.content1_with
    assert_equal "CONTENT2", tab.content2_benefits
    assert_equal "CONTENT3", tab.content3_training
  ...
  def test_set_contents_legacy
    ...
    tab = Factory(:tab, :user => user, :title => "Yoga", :content => content )
    tab.reload
