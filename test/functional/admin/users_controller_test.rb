...
class Admin::UsersControllerTest < ActionController::TestCase
  ...
  def test_update
    ...
    cyrille = users(:cyrille)
    post :update, {:id => norma.slug, :user => {:admin => false, :main_role => "Free listing", "member_since(1i)" => "2008",
  ...
  def test_update_free_to_full_member
    ...
    cyrille = users(:cyrille)
    post :update, {:id => rmoore.slug, :user => {:admin => false, :main_role => "Full member", "member_since(1i)" => "2008",
       "member_since(2i)" => "10", "member_since(3i)" => "30", "member_until(1i)" => "2009",
          "member_until(2i)" => "11", "member_until(3i)" => "1" }  }, {:user_id => cyrille.id}
    assert_redirected_to search_admin_users_path
    rmoore.reload
