module CryptoPaypal
  class Button
    def initialize(data)
      my_cert_file = Dir.getwd + "/paypal/#{CryptoPaypal::Config.public_certificate}"
      my_key_file = Dir.getwd + "/paypal/#{CryptoPaypal::Config.private_key}"
      paypal_cert_file = Dir.getwd + "/paypal/#{CryptoPaypal::Config.paypal_certificate}"
