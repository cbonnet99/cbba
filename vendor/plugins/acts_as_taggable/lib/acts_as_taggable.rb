  ...
  module Acts #:nodoc:
    module Taggable #:nodoc:
      ...
      module InstanceMethods
        def tag_with(company_id, list)
          Tag.transaction do
            taggings.destroy_all

            Tag.parse(list).each do |name|
              if acts_as_taggable_options[:from]
                send(acts_as_taggable_options[:from]).tags.find_or_create_by_company_id_and_name(company_id, name).on(self)
              else
                Tag.find_or_create_by_company_id_and_name(company_id, name).on(self)
              end
            end
          end
        end
